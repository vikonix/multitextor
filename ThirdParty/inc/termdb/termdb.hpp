#pragma once
//#define TERMDB_IMPLEMENTATION put this define in one of .cpp file
...
// param types
using param = std::variant<long, std::string>;
...
public:
    ...
    std::optional<uint16_t> get(tdb::num _n) const noexcept
    {
                           ...
                           param p9) const
{
    ...
    public:
        std::optional<long> popNum() noexcept
        {
            if (!stk.empty()) {
                auto &element = stk.top();
                try {
                    auto num = std::get<long>(element);
                    stk.pop();
                    return num;
        ...
        std::optional<std::string> popString() noexcept
        {
            if (!stk.empty()) {
                auto &element = stk.top();
                try {
                    auto num = std::get<std::string>(element);
                    stk.pop();
                    return num;
                } catch (std::bad_variant_access &e) {
                    return {};
        ...
        std::optional<param> pop() noexcept
        {
            if (!stk.empty()) {
    ...
    for (decltype(strLength) i = 0; i < strLength; ++i) {
        ...
        switch (s[i]) {
            ...
            case 'i': {
                try {
                    auto &param1 = std::get<long>(p1);
                    auto &param2 = std::get<long>(p2);
                    ++param1;
                    ++param2;
                } catch (std::bad_variant_access &e) {
                    incorrectString = true;
